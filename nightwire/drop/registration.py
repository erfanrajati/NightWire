...
class DropModule:
    ...
    def register(self, registry: ApplicationRegistry, context: ModuleContext) -> None:
        ...
        registry.add_route("/drop/{filename:str}", handler("drop_share_page"), methods=["GET"])
        registry.add_route("/api/drops", handler("list_active_drops"), methods=["GET"])
        registry.add_route("/api/drops/files", handler("upload_file"), methods=["PUT"])
        registry.add_route("/api/drops/text", handler("share_drop_text"), methods=["POST"])
        registry.add_route("/api/drops/{filename:str}", handler("drop_share_info"), methods=["GET"])
